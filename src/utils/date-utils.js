export const mDate = (dateString) => {
  let date = dateString ? new Date(dateString) : new Date();

  let dualize = (x) => (x < 10 ? "0" + x : x);
  let getTime = () =>
    dualize(date.getHours()) + ":" + dualize(date.getMinutes());
  let getDate = () =>
    dualize(date.getDate()) +
    "/" +
    dualize(date.getMonth()) +
    "/" +
    dualize(date.getFullYear());
  ...
  return {
    ...
    lastSeenFormat: () => {
      ...
      let value =
        dateDiff === 0 ? "today" : dateDiff === 1 ? "yesterday" : getDate();
